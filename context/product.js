...
export const ProductProvider = ({ children }) => {
    ...
    const uploadImages = (e) => {
      ...
      if (files) {
        ...
        if (totalImages > 10) {
          toast.error("Se pueden subir hasta 10 imagenes");
          return;
        }
        setUploading(true);
    ...
    const createProduct = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                toast.success(`Producto "${data?.title}" creado`);
    ...
    const updateProduct = async () => {
        try {
          ...
          if (!response.ok) {
            ...
            toast.success(`Producto "${data?.title}" actualizado`);
            ;
          }
        } catch (err) {
          console.log(err);
    ...
    const deleteProduct = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          toast.success(`Producto "${data?.title}" eliminado`);
        }
      } catch (err) {
        console.log(err);
      }
